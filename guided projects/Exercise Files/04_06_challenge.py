...
class Canvas:
    ...
    def hitsWall(self,point):
        return self.hitsWallHorizontalWall(point) or self.hitsWallVerticalWall(point)
    def hitsWallVerticalWall(self, point):
        return round(point[0]) < 0 or round(point[0]) >= self._x
    def hitsWallHorizontalWall(self, point):
        return round(point[1]) < 0 or round(point[1]) >= self._y
    
    def getReflection(self, point):
            return [ -1 if self.hitsWallVerticalWall(point) else 1,-1 if self.hitsWallHorizontalWall(point) else 1]
...
class TerminalScribe:
    ...
    def drawSquare(self, size):
        ...
        for i in range(size):
            self.up()
    #needed to set the starting position if we define it outside the class an not use the default one
...
scribe.plotX(scribe.sine)
scribe.plotX(scribe.cosine)